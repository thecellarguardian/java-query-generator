...
extends SyntacticAnalyzer<PAQLTokenClass, PAQLParseTreeClass>
{
    ...
    )
    {
        ...
        ListIterator< Token<PAQLTokenClass> > probe = (ListIterator< Token<PAQLTokenClass> >)tokenIterator;
        while
        (
            (expect(PAQLTokenClass.KEY_KEYWORD, checkedNext(probe)))
        )
        {
            probe.previous();
            declarationBlock.add(parseKeyDeclaration(probe));
        }
        probe.previous();
        if
        (
            (!expect(PAQLTokenClass.RIGHT_CURLY_BRACKET, checkedNext(probe)))
        )
        {
            probe.previous();
            declarationBlock.add(parseVariableDeclarationBlock(probe));
        }
        Token<PAQLTokenClass> rightCurlyBracket = checkedNext(probe);
        if
        (
            (!expect(PAQLTokenClass.RIGHT_CURLY_BRACKET, rightCurlyBracket))
        )
        {
            throw new RuntimeException
            (
                ...
                "\n\tline:"
                +
                rightCurlyBracket.getLineNumber()
                +
                "\n\terror: '}' expected"
            );
        }
    ...
    )
    {
        ...
        keyDeclaration.add(parseVariableDeclarationBlock(tokenIterator));
        return keyDeclaration;
    }
    private ParseTree<PAQLParseTreeClass> parseVariableDeclarationBlock
    (
        Iterator< Token<PAQLTokenClass> > tokenIterator
    )
    {
        System.out.println("\t\t\tParsing declaration block");
        VariableDeclarationBlock variableDeclarationBlock = new VariableDeclarationBlock();
        Token<PAQLTokenClass> leftCurlyBracket = checkedNext(tokenIterator);
        if(!expect(PAQLTokenClass.LEFT_CURLY_BRACKET, leftCurlyBracket))
        {
            throw new RuntimeException
        ...
        )
        {
            ...
            variableDeclarationBlock.add(parseVariableDeclaration(blockEndProbe));
        }
